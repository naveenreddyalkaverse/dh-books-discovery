...
const customAnalyzer = function (tokenizer) {
    let filters = arguments.length <= 1 || arguments[1] === undefined ? [$LowercaseFilter] : arguments[1];
    return { type: 'custom', tokenizer: tokenizer.name, filter: _.map(filters, filter => filter.name) };
};
