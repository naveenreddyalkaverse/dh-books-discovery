...
const BeaconConfig = new Config('dh-books/beacon', Path.resolve(__dirname, '../beaconConfig'));
...
export default class Beacon {
    ...
    wrapEventProperties(headers, eventType, event) {
    ...
    send(headers, eventType, event) {
        const eventRecord = this.wrapEventProperties(headers, eventType, event);
    ...
    //noinspection JSMethodCanBeStatic
    searchQueryEventProperties(queryData, queryLanguages) {
        return {
    ...
    sendSearchResult(data) {
        ...
        this.send(data.headers, SEARCH_RESULT_EVENT, _.assign(this.searchQueryEventProperties(data.queryData, data.queryLanguages), eventProperties));
    }
}